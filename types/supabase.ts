...
// Tipos para os joins e consultas complexas
...
export interface RelatorioFinanceiro {
  ...
  receitaPorTipoVeiculo: { tipo: string; valor: number; percentual: number }[]
  receitaPorFormaPagamento: { forma: string; valor: number; percentual: number }[]
  transacoesRecentes: TicketCompleto[]
}
...
export interface RelatorioOcupacao {
  ...
  tempoMedioPermanencia: {
    geral: number // em minutos
    porTipoVeiculo: { tipo: string; duracao: number }[]
  }
  horasPico: { dia: string; hora: string }[]
  ocupacaoPorTipoVeiculo: { tipo: string; quantidade: number; percentual: number }[]
}
