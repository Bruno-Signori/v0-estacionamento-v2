...
export function VehicleExitDetails({ vehicleInfo, pricingTables, onFinishExit }: VehicleExitDetailsProps) {
  ...
  const handleFinishExit = () => {
    ...
    setTimeout(() => {
      ...
      // Chamando a função de finalização após um breve delay para mostrar a confirmação
      setTimeout(() => {
  ...
  const handlePrintReceipt = () => {
    // Em um sistema real, isso enviaria os dados para impressão
    window.print()
  }
        ...
        <CardContent className="p-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Button
                className="rounded-xl bg-green-500 text-white hover:bg-green-600 focus:ring-green-500 h-12"
                disabled={
                  (amountToPay > 0 && !selectedPaymentMethod) || isProcessing || !selectedPricingTable || exitConfirmed
                }
              ...
              <Button
                variant="outline"
                className="rounded-xl border-gray-300 text-gray-700 hover:bg-gray-50 h-12"
